...
The idea behind `inbreedR` is to provide a consistent framework for the analysis of inbreeding and heterozygosity-fitness correlations (HFCs) based on genetic markers. The package is in a very early state and we are happy about any comments and suggestions. Just write a mail to martin.adam.stoffel[at]gmail.com.


## Installation

The `inbreedR` package is not yet available on [CRAN](http://cran.r-project.org/). Therefore you have to download the newest version from the developer platform [GitHub](https://github.com/mastoffel/inbreedR). At the moment it is recommended that you frequently download and reinstall it, as it is under heavy development. To download and install the package plus this vignette, run the following code (you need R version 3.2.2 or newer to run it).
...
The data format used by all functions is fairly simple. The input is always a `data.frame` or `matrix`, whereby rows represent individuals, and each column represents a locus. If an individual is heterozygous at a certain locus, it is coded as `1` while homozygote loci should be `0`. Missing data should be encoded as NA. `mouse_snps` is a sample dataset accompanying the package. 
...
`convert_raw` is a function to convert a more common format, where each locus is represented by two coloumns (alleles), into the inbreedR working format. Microsatellite data is often formatted like `mouse_msats`, which is the second dataset accompanying the package.
...
The dataframe itself should just contain the marker data, while rownames for individuals or columnnames for loci are possible. To convert it into the `inbreedR` working format, just use the `convert_raw` function.

```{r}
mouse_msats_convert <- convert_raw(mouse_msats) 
mouse_msats_convert[1:8, 1:8]
```

It is now converted to the working format (0 = homozygote, 1 = heterozygote, NA = missing). The same procedure works when
...
Before we start with a step by step guide to the functions in the package, we will start with some background on how the `inbreedR` functions are embedded in the current theoretical and empirical framework of inbreeding and HFCs. You can skip this section if you are familiar with the theory and directly want to dive into the `inbreedR` manual.
...
Several highly intercorrelated measures of heterozygosity have historically been used in HFC studies, while `inbreedR` solely relies on standardized multilocus heterozygosity (function: `sMLH`) [@coltman1999parasite]. Although all heterozygosity measures are usually highly intercorrelated, sMLH is in general the most robust measure [@Chapman:2009vmba]. This has the reason that it is not based on allel frequencies which makes it less sensitive towards scoring errors and null alleles than other measures [@Szulkin:2010dxbaca]. Besides this robustness, it is one of the most commonly used measures [@kardos2014evaluating] and proved to be computationally much faster than allel-based measures which makes it applicable even to permutation and bootstrapping procedures in large datasets.
...
**$g_2$** measures the excess of double heterozygote loci in a population. The original formula by [@david2007reliable] is implemented in the `g2_microsats` function, which also allows calculation of a confidence interval based on bootstrapping and a p-value based on permutation tests. The function arguments `nperm` and `nboot` specify the number of permutations for estimating a p-value and the number of bootstraps for estimating a confidence interval, respectively. CI specifies the range of the confidence interval.

```{r, results = "hide"}
g2_mouse <- g2_microsats(mouse_msats_convert, nperm = 100, nboot = 100, CI = 0.95)
```

A summary of the results is shown by simply typing the object name.

```{r}
g2_mouse
```

Every output (except function sMLH) of an `inbreedR` function is an `inbreed` object. You can use the `plot` function to show the distribution of the bootstrapped values and a confidence interval.
...
```{r}
plot(g2_mouse, main = "just another title", xlab = "identity disequilibrium (g2)", 
     breaks = 5, col = "red")
```
...
The computation time difference becomes clear already on a standard computer. When `inbreedR` is loaded,
you can try the following on you computer:
...
For a summary of the results and a plot, just follow the same procedure as with the g2_microsat function.
...
HHC is another way to quantify variance in inbreeding. The genetic markers are randomly split into two equal parts and the correlation between the resulting standardized multilocus heterozygosity (sMLH) values is calculated. This is repeated over n iterations. The resulting values ranges from 0 (no differences in inbreeding-level) to 1 (large variance in inbreeding-level). The `HHC` functions takes as input the marker data, the number of iterations and the size of the confidence interval to calculate around the mean HHC.

```{r, results = "hide"}
hhc_microsats <- HHC(mouse_msats_convert, niter = 100, CI = 0.95)
```

By simply typing the object we get a summary again.

```{r}
hhc_microsats
```

A histogram of the resulting HHC distribution can be shown with `plot`.
...
## sMLH
Several highly intercorrelated measures of heterozygosity have historically been used in HFC studies, while `inbreedR` solely relies on standardized multilocus heterozygosity (sMLH) [@coltman1999parasite]. Although all heterozygosity measures are usually highly intercorrelated, sMLH is in general the most robust measure [@Chapman:2009vmba]. This has the reason that it is not based on allel frequencies which makes it less sensitive towards scoring errors and null alleles than other measures [@Szulkin:2010dxbaca]. Besides this robustness, it is one of the most commonly used measures [@kardos2014evaluating].  
The package provides a computationally optimized function for the calculation of sMLH [@Coltman:1999wxba]), which can be used within bootstrapping and permutation tests also for large datasets. The function simply takes the genotypes in the `inbreedR` working format as input and gives a simple vector output with each individual's sMLH.
```{r}
het <- sMLH(mouse_snps)
hist(het)
```

It just takes a quarter of a second to calculate it on a standard laptop for out SNP dataset.
...
As the important parameters around the left-hand side of the equation can be estimated directly with a `glm` in R, effect of inbreeding level (f) on heteorzygosity (h) and fitness (W) has to be derived by appropriate formulae.
...
A typical way to show the $g_2$ distributions for different subsets of markers are boxplots.  Using the plot function on `inbreed` objects that involve different sized subsets shows boxplots.
