...
Subsampling analysis [@Miller:2013jqbaca; @stoffel2015chemical] can provide insights into the power provided by a given marker panel. 
  
For that reason, the sensitivity of $\hat{r}^2(h, f)$ towards the number of markers used can be explored using the `r2_hf` function, which allows specification of the marker subsets and how often each subset should be sampled. 
...
So far, the uncertainty of a $g_2$ estimate is assessed via bootstrapping and confidence intervals. However, for planning future studies it might be of interest how the uncertainty of $g_2$ changes by increasing or decreasing the number of genetic markers.
