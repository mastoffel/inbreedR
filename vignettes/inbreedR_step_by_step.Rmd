...
So far, the uncertainty of $g_2$ and other estimates is assessed via bootstrapping and confidence intervals. However, for planning future studies it might be of interest how the uncertainty of $g_2$ changes by increasing or decreasing the number of genetic markers.`simulate_g2` allows to explore the precision of $g_2$ by drawing a number of markers repeatedly and independently from a simulated "genome". Here, we use an individual based simulation that assumeslinkage equilibrium between loci, random mating and the same expected
heterozygosity for all loci. Genotypes are stochastically assigned and depend on the inbreeding level *f* of the simulated individuals. The user also defines the heterozygosity of non-inbred individuals at 
the simulated loci (`H_nonInb`), the mean (`meanF`) and variance (`varF`) of *f* in the 
population, the number of repetitions per subset (`reps`), and the number of 
individuals (`n_ind`). The `type` argument specifies the $g_2$ formula to use. With `type = snps`,
...
```{r, echo=TRUE, results='hide'}
sim_r2_hf_mouse_microsats <- simulate_r2_hf(n_ind = 20, H_nonInb = 0.5, meanF = 0.2, varF = 0.05,
                                            subsets = c(4,6,8,10,12), reps = 100, 
                                            type = "msats", CI = 0.95)
```
