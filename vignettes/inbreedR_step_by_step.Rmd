...
author: "Martin A. Stoffel, Mareike Esser, Hazel Nichols, Marty Kardos, Patrice David, Joe Hoffman"
date: "`r Sys.Date()`"
...
The idea behind `inbreedR` is to provide a consistent framework for the analysis of inbreeding and heterozygosity-fitness correlations (HFCs) based on genetic markers. We are happy about any suggestions and feedback. Just write a mail to martin.adam.stoffel[at]gmail.com.
...
* `r2_hf`: Calculates the expected impact of inbreeding-level (f) on heterozygosity (h) with a formula derived by @szulkin2010heterozygosity.

* `simulate_g2`: Simulates $g_2$ values based on independently drawn sets of genetic markers. The number of individuals, marker number and the distribution of heterozygosity can be specified by
the user or is automatically calulated from given empirical genotypes. This simulation can be used to estimate the precision of g2 for a given dataset.
...
Note: For plotting the histogram with confidence interval for `r2_hf` you have to specify an additional argument to the
`plot` function, because there will be another plotting possibility for the output of this function as you will read later in this vignette.
...
So far, the uncertainty of $g_2$ and other estimates is assessed via bootstrapping and confidence intervals. However, for planning future studies it might be of interest how the uncertainty of $g_2$ changes by increasing or decreasing the number of genetic markers.
`simulate_g2` allows to explore the precision of $g_2$ by drawing a number of markers repeatedly and independently from a simulated "genome". Next to the number of individuals and the size of marker sets, the user can specify the heterozygosity distribution for the simulated loci. This can happen directly, with the `mean_MLH` argument (should be a number between 0 and 1, whereby 0.5 would mean that on average 50% of the loci ar heterozygous) and the `sd_MLH` argument (standard deviation of multilocus heterozygosity), or will be automatically calculated from empirical genotypes, that can be provided with the `genotypes` argument. The simulation assumes (1) random mating, (2) unlinked loci and (3) the same expected heterozygosity across all loci. Below we run a simulation with 20 individuals for marker sets ranging from 4 to 12 markers, while each set of markers is drawn 100 times.

```{r, echo=TRUE, results='hide'}
sim_g2 <- simulate_g2(n_ind = 20, subsets = c(4,6,8,10,12), reps = 100, 
                      genotypes = mouse_microsats, type = "msats", CI = 0.95)
```
...
The same simulation can be used to estimate the expected squared correlation between inbreeding and heterozygosity for an increasing number of markers
with the `simulate_r2_hf` function as shown below. 

```{r, echo=TRUE, results='hide'}
sim_r2_hf_mouse_microsats <- simulate_r2_hf(n_ind = 20, mouse_microsats, subsets = c(2,4,6,8,10,12), 
                           reps = 100, genotypes = mouse_microsats, type = "msats")
```

```{r r2_hf, echo=FALSE, fig.cap="Expected r2 between inbreeding level (f) and heterozygosity", fig.width=7, fig.height=4}
plot(sim_r2_hf_mouse_microsats, main = "Microsatellites", cex.axis=0.85)
```
