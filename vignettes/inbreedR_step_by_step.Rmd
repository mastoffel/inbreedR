...
* `simulate_g2`: Simulates g2 values based on independently drawn sets of genetic markers. The number of individuals, marker number and the destribution of heterozygosity can be specified by
the user. This simulation can be used to estimate the precision of g2 for a given dataset.

* `simulate_r2_hf`: Works equivalent to `simulate_g2`. However the estimates are the expected squared correlations between inbreeding and heterozygosity.
