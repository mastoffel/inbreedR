...
The idea behind `inbreedR` is to provide a consistent framework for the analysis of inbreeding and heterozygosity-fitness correlations (HFCs) based on genetic markers. This vignette gives a practical introduction into the concept of the package and how to use the functions. For a more concise theoretical background and a citeable source please read our paper. We are happy about any suggestions and feedback on the package. Just write a mail to martin.adam.stoffel[at]gmail.com.
...
The same simulation can be used to estimate the expected squared correlation between inbreeding and heterozygosity for an increasing number of markers with the `simulate_r2_hf` function as shown below. 
